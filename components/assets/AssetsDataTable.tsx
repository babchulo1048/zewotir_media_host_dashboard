...
const assetColumns: (handlers: {
  ...
  {
    accessorKey: "thumbnailUrl", // ✅ camelCase
    header: "Preview",
    cell: ({ row }) => {
      const thumbnailUrl = row.original.thumbnailUrl; // ✅ camelCase
      const title = row.original.title;
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="font-medium">
        {row.getValue("title")}
        <div className="text-sm text-muted-foreground">
          {row.original.assetType?.toUpperCase() || "N/A"} {/* ✅ camelCase */}
        </div>
      </div>
    ),
  },
  {
    accessorKey: "isFeatured", // ✅ camelCase
    header: "Featured",
    cell: ({ row }) => {
  ...
  {
    accessorKey: "isActive", // ✅ camelCase
    header: "Status",
    cell: ({ row }) => {
